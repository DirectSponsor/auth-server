...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['email'])) {
    ...
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if (checkRateLimit($email, 'verification', 3, 3600)) {
            ...
            try {
                ...
                if ($user && !$user['email_verified']) {
                    $token = generateToken();
                    $expires = date('Y-m-d H:i:s', time() + VERIFICATION_TOKEN_EXPIRY);
                    $stmt = $db->prepare("UPDATE users SET verification_token = ?, verification_token_expires = ? WHERE id = ?");
                    ...
                    $message = 'If that email is registered and unverified, you will receive a verification email.';
                    $message_type = 'success';
                }
            } catch (Exception $e) {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .container { background: white; border-radius: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); max-width: 400px; width: 100%; padding: 40px; }
        h1 { color: #333; margin-bottom: 10px; font-size: 24px; }
        .subtitle { color: #666; margin-bottom: 30px; font-size: 14px; }
        .form-group { margin-bottom: 20px; }
        label { display: block; color: #555; margin-bottom: 8px; font-weight: 500; }
        input[type="email"] { width: 100%; padding: 12px; border: 2px solid #ddd; border-radius: 6px; font-size: 14px; }
...
<body>
    <div class="container">
        ...
        <div class="back-link"><a href="https://roflfaucet.com/">Back to site</a></div>
    </div>
</body>
</html>
